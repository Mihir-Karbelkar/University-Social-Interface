...
$.ajax(
{
	...
	success: function(data, textStatus, xhr){
		...
		$('<table><tr><th>ID</th> <th>Course Code</th> <th>Subject</th> <th>Course Credits</th> <th>Attendance</th> <th>Attendance(%)</th> </th>').appendTo('#container')
		for(i=0,len=data["lis"].length;i<len;i++){
			$('<tr><td>'+ data["lis"][i]["id"]+'</td><td>'+ data["lis"][i]["code"]+'</td><td>'+ data["lis"][i]["subject"]+'</td><td>'+ data["lis"][i]["credit"]+'</td><td>'+ data["lis"][i]["total"]+'</td><td>'+ data["lis"][i]["percent"]+'</td></tr>').appendTo('table');
		}
...
$.ajax({
	...
	data: person2,
	success: function(data, textStatus, xhr){
		$('h3').eq(0).html('Events to be completed<i class="material-icons">event</i>');
		...
		$('h3').eq(1).html('Events in this month<i class="material-icons">calendar_today</i>');
			...
			$('h3')[1].after('<p>No events anytime soon.</p>');
					...
					var rt1 = "";
						...
						for(i=0, len=data["text"].length;i<len;i++){
							...
							rt1+="<li><a value='"+data["links"][i]+"' class='todo'>"+data['text'][i]+"</a></li>"
...
'mood_passw' : $('div')[3].getAttribute('value') }
	...
	beforeSend : function(){
		$(event.target).children('.loader').show();
	},
	complete: function(){
		$(event.target).children('.loader').hide();
